...
对抗性审查的最后一批——25 条 medium/low。其中 5 条不成立，而**一条标着 low 的
恰恰是整轮审查里最严重的缺陷**。

### 修复

- **信用分的方向是反的。** `ScorecardBuilder.score` 把 `coef * WOE * factor`
  直接相加而没有取负。WOE 是 `ln(坏/好)`，加权和越大表示申请人越差——于是分数
  随风险上升。实测与坏概率的相关是 **+0.986**，而同一个模块里的
  `probability_to_credit_score` 方向是对的。**任何"高分放款"的策略切分建在它
  上面，都会把最坏的客户放出去。** 这条被标为 low，它不是。

- **`ScorecardBuilder` 从未使用模型截距。** 截距在 fit 时被取出、存下，然后
  再也没用过，`predict_proba` 因此与底层模型差一个 log-odds 常数。现在截距计入
  基准分，`predict_proba` 与模型完全一致（`round_scores=False` 时；开启取整后
  的残差是整数量化误差）。

- **`DataPreprocessor` 的统计量不使用样本权重。** 那个用来填充所有缺失申请人的
  均值/中位数——训练时和保存的 artifact 里都用它——描述的是样本而非总体。在一个
  总体由高值组主导的样例上，加权后中位数从 50.5 变成 100.0。

- **Lift/十分位表不加权。** `qcut` 数的是行数，欠采样下这张风控最常看的表里，
  "十分位"实际装了 0.6% 和 19%。占比极差从 0.184 降到 0.0005；不加权路径仍走
  原来的 Polars qcut。

- **`custom_null_values` 在 transform 里生效、在 fit 里被忽略。** 哨兵值被平均
  进了那个稍后要替换它的统计量：以 -999 表示"未知"时，`[1, 2, -999, 4, -999]`
  的中位数算成 1.0 而非 2.0，然后每个缺失行都被填成这个歪掉的值。

- **`split_dev_oot` 静默丢行。** 样本列既不是 Dev 也不是 OOT 标签的行不参与
  训练与评估。这常常是有意的，但现在会打日志告知数量和具体取值。

- **运行归档写出的是非法 JSON。** NaN/Infinity 指标被序列化成 `NaN`/`Infinity`，
  Python 读得回来，但任何严格 JSON 解析器都会报错。现在存成 `null`。

- **`runs compare` 把变成 NaN 的指标显示成"未变化 `(=)`"。** 一个 AUC 变成 NaN
  的模型，看起来和没动过一模一样。

- **`resolve_run` 会优先取当前目录下的同名文件夹**，而不是真正的归档，于是一个
  无关的本地目录能悄悄把错误的运行喂给对比。现在归档优先。

### 文档

七处与代码矛盾的表述已修正：参考页缺两个 CLI 参数、测试数过期、"每个配置项都能
被 CLI 覆盖"（有两个不能）、加载器会读但示例里没有的配置键、算法列表漏了
CatBoost、`Monitor.monitor` 两个未文档化的参数、输出目录树漏了 `model_report/`。

### 不成立

5 条未能复现：`runs list` 本就容错坏目录、PSI 空箱的 epsilon 贡献确为 0.0、
`ModelTrainer` 调参的评分器本就加权。

## 3.3.1（2026-08-15）

### 新增

- **补上三个变异测试发现无保护的行为。** 故意把这三处改坏，190 个测试**全绿**：
  - 流水线改成在 Dev+OOT 上拟合预处理——**整个项目赖以立身的防泄漏保证，
    在流水线层面没有任何测试覆盖**，此前只测了 `DataPreprocessor` 单独的行为
  - `ScorecardBuilder` 硬编码 factor、无视配置的 `PDO`
  - `probability_to_credit_score` 忽略传入的 `pdo` 参数

    现在三个变异体全部被杀死。测试数 193。

## 3.3.0（2026-08-15）

### 新增

- **`clean_strategy: "keep"` —— 让缺失走到 WOE。** `AutoPipeline` 总是先填充
  再分箱，因此 `WoeBinner` 的 Missing 箱对入模特征永远不可达，"缺失 vs 有值"
  的对比被静默丢弃。而在信贷里这个对比常常是一个特征最强的信号——
  "查不到征信记录"的人不是一个中位数申请人。

    实测：缺失率 25% 且缺失组坏率 0.60（其余 0.14）的样例上，
    填充后 IV = 0.653 且没有 Missing 箱；用 `keep` 后 IV = 1.020，
    Missing 箱 WOE = 1.44，**OOT AUC 从 0.676 升到 0.716**。

    `keep` 在分类流水线里是安全的：WOE 会把每个箱（含 Missing）都映射成数值，
    模型拿到的不含缺失。

## 3.2.0（2026-08-15）

对抗性审查的最后一批。六个缺陷，其中五个共享同一个主题：**样本权重在各个环节
的含义并不一致**。这恰好咬在信贷建模最核心的地方——欠采样好客户、拒绝推断——
所以此前用权重训练过的模型建议重新拟合。

### 修复

- **分箱切点完全不使用样本权重。** quantile、CART 以及顺序 CART 回退路径都在
  原始行上拟合切点。当好客户按 1:50 欠采样时，所谓"等频"分箱实际装了加权总体的
  39%/24%/18%/13%/7%（而非各 20%），CART 切的也是样本而不是总体。现在切点由
  加权分位数决定，决策树也真正收到 `sample_weight`——切点与直接调用 sklearn
  完全一致。不加权时仍走原来的 Polars 快速路径，行为不变。

- **WOE/IV 的平滑常数受权重量纲影响。** 固定的 0.5 被加到加权求和上，于是把所有
  权重乘以同一个常数——这不改变它们的相对含义——却让 IV 变化 1.6 倍。现在平滑量
  以"平均权重"为单位表达，IV 与量纲无关，不加权时数值完全不变。

- **PSI 的分箱边界取自未加权的基准。** 欠采样下这些"十分位"实际装了加权总体的
  1%~19%，PSI 的分辨率全堆在被过采样的尾部，而在总体真正所在处几乎是盲的。
  现在边界切分加权基准，实测占比极差从 0.177 降到 0.0006。

- **`RegressionPipeline` 忽略 `use_sample_weight`。** 它只看 `weight_col` 是否
  存在，于是随包发布的配置——同时写着 `use_sample_weight: false` 和
  `weight_col: weight`——会用同一份文件训出加权的回归和不加权的分类。在一个
  权重会翻转关系的样例上，标志位名义关闭时系数从 -0.17 变成 +4.82。

- **`log1p` 回归配 `early_stopping_eval='oot'` 时验证量纲错误。** 模型拟合的是
  `log1p(y)`，但 OOT 验证集拿到的是原始目标，于是早停在拿 log 尺度的预测比原始
  尺度的标签：验证曲线是 14.215 → 13.921（平坦噪声）而不是 0.567 → 0.270，
  树的数量由噪声决定。而 `docs/guide/regression.md` 演示的正是这个组合。

- **`ScorecardBuilder.score()` 传 NumPy 数组时返回常数。** 数组分支用模型的
  `_bin` WOE 列名去标注原始驱动列，分箱器匹配不到任何列，每一行都塌缩成
  `offset_`——137 个不同分数变成 1 个，且不报任何错。现在数组路径使用分箱器
  拟合时的驱动名，列数不符会明确拒绝。

### 新增

- `auto_modeling_tool.core.stats` —— `weighted_quantile` / `weighted_mean`，
  由分箱与 PSI 共用。均匀权重直接委托给 `np.quantile`，保证不加权路径逐位不变。
- `tests/test_weight_contract.py` —— 12 个测试锁住上述每一条，其中包括把 CART
  切点与"加权的 sklearn"直接对照。
